...
// TOOLTIPS:
var tooltip = d3.select('body').append('div').attr('class', 'tooltip').style('width', '100');

//select all points
d3.selectAll(Grob).selectAll('use').attr('class', 'point');
...
d3.selectAll('.point')
	...
	.on('click', function (d, i) {
		...
		d3.selectAll('.point').attr('class', function () {
...
$('#table tbody').on('click', 'tr', function () {
	...
	d3.selectAll('.point').attr('class', function (d, i) {
	...
	switch (t) {
		...
		case 'quadratic':
			return (
				...
				(trendInfo[t][1] > 0 ? '+' + trendInfo[t][1] : trendInfo[t][1]) +
				'x ' +
				(trendInfo[t][2] > 0 ? '+' + trendInfo[t][2] : trendInfo[t][2]) +
				'x<sup>2</sup>' +
				'</span>' +
				'<br />' +
				'R' +
		...
		case 'cubic':
			return (
				...
				(trendInfo[t][2] > 0 ? '+' + trendInfo[t][2] : trendInfo[t][2]) +
				'x<sup>2</sup> ' +
				(trendInfo[t][3] > 0 ? '+' + trendInfo[t][3] : trendInfo[t][3]) +
				'x<sup>3</sup> </span>' +
				'<br />' +
				'R' +
				'<sup>2</sup> : <span>' +
...
addLineInteraction = function (t, g, ptip) {
	d3.select(legLineLayout)
		...
		.on('click', function () {
			d3.selectAll('.trend').classed('hidden', function () {
				return this.getAttribute('class').includes(t) == true ? false : true;
			});

			return ptip.style('display', null).html(g);
		});
...
d3.select(pp).insert('g', 'g:nth-child(4)').attr('class', 'brush').call(brush);

// make handles invisible:
d3.selectAll('.handle').style('opacity', 0);
d3.select('.overlay').attr('width', '100%').attr('height', '100%');
d3.select('.overlay').style('stroke', 'none');
...
//Reset Button:
reset = function () {
	d3.selectAll('.point').classed('none selected', false);
	...
	d3.selectAll('.trend').classed('hidden', false);

	d3.select('.trend-info').classed('hidden', true);
};
