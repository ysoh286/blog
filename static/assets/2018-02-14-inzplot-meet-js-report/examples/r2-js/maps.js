...
var svg = d3.select('svg').attr('width', null).attr('height', null);
...
var data = chart.data,
	...
	regions = d3
		.selectAll(pathElements)
		.attr('class', 'region')
		.attr('id', function (d, i) {
			return data[i][names[0]] + '.' + i;
		}),
	sparkRegions = d3
		.selectAll(sparkElements)
		.attr('class', 'sparkRegion')
		.attr('id', function (d, i) {
			return 'sparkRegion.' + i;
		}),
	centroids = d3.select('svg').selectAll('use').attr('class', 'centroid'),
	tooltip = d3.select('body').append('div').attr('class', 'tooltip').style('width', '100'),
	defaultColours = [
		'#101f33',
		'#162e47',
		'#1b3c5e',
...
makeTooltips = function (chart) {
	...
	d3.selectAll('.region')
		...
		.on('click', function (d, i) {
			...
			d3.selectAll('.region').attr('class', function (d, i) {
			...
			// for point maps:
			d3.selectAll('.centroid').attr('class', function (d, i) {
				if (!d3.event.shiftKey) {
...
filterData = function (data, group, region) {
	...
	for (i = 0; i < data.length; i++) {
		// to take into account of missings
...
tableToPlot = function (type) {
	...
	$('#table tbody').on('click', 'tr', function () {
		...
		d3.selectAll('.' + type).attr('class', function () {
			var id = this.id;
...
reset = function () {
	//clear map
	d3.selectAll('.region').classed('selected none', false);

	d3.selectAll('.sparkRegion').classed('.sparkRegion none', false).classed('hidden', false);

	d3.selectAll('.centroid').classed('.centroid none', false);
	...
	d3.select('.spark-div').classed('hidden', true);

	//reset zooming:
	d3.select('.background').attr('transform', null);
};
...
sparkPlot = function (chart) {
	...
	//yaxis:
	g.append('g').attr('class', 'spark-yaxis').call(d3.axisLeft(yScale));

	// for lines:
	g.append('g').attr('class', 'line-group').append('path');
	...
	var xlab = spark
		...
		.attr(
			'transform',
			'translate(' + width / 1.75 + ',' + (height + margin.bottom + margin.top) + ')'
		)
	...
	var lineTool = g.append('g').attr('class', 'lineTool').style('display', 'none');
	...
	lineTool.append('circle').attr('class', 'follow-path');

	lineTool.append('text').attr('class', 'follow-text');
...
setLegend = function () {
	...
	var newScale = d3.selectAll(layout).append('g').attr('id', 'new-scale').classed('hidden', true);

	var defs = newScale.append('defs');

	var linearGradient = defs.append('linearGradient').attr('id', 'linear-gradient');

	linearGradient.attr('x1', '0%').attr('x2', '0').attr('y1', '0%').attr('y2', '100%');
...
updateFills = function () {
	...
	d3.selectAll(ss).classed('hidden', true);

	// divide domain according to number of colours supplied.
	var dval = [];
	var min = d3.min(dd, function (d) {
		return d[yvar];
	});
	var max = d3.max(dd, function (d) {
		return d[yvar];
	});
	...
	colorScale = d3.scaleLinear().range(pal).domain(dval);
	...
	var stops = linearGradient
		.selectAll('stop')
		.data(dval)
		.enter()
		.append('stop')
		.attr('offset', function (d, i) {
			return (i / dval.length) * 100 + '%';
		})
	...
	var legAxis = d3.scaleLinear().range([100, 0]).domain([min, max]);

	var leg = d3.select('.leg-axis').call(d3.axisLeft(legAxis).ticks(4).tickFormat(d3.format('d')));

	// attempt to mimic 'ggplot2' legend:
	leg.selectAll('path').attr('stroke', 'none');

	leg.selectAll('line').attr('stroke', 'white');

	leg.selectAll('text').attr('x', 1).attr('font-size', 7).style('text-anchor', 'start');

	d3.select('#new-scale').classed('hidden', false);

	// update legend title:
	d3.select('.leg-title').text(yvar);
...
updateSpark = function (data, selectedRegion, xvar, yvar) {
	...
	d3.select('.spark-yaxis').call(d3.axisLeft(yScale));

	d3.select('.spark-title').text(yvar + ' by ' + xvar);

	d3.select('.y-lab').text(yvar);

	d3.select('.spark-div').classed('hidden', false);

	// line function
	addLines = d3
		.line()
		.x(function (d) {
			return d[xvar] ? xScale(d[xvar]) : xScale(0);
		})
		.y(function (d) {
			return d[yvar] ? yScale(d[yvar]) : yScale(0);
		});
	...
	var textLabels = lineTool
		.selectAll('.follow-text')
		.data(data)
		.enter()
		.append('text')
	...
	mousemove = function () {
		...
		if (circleData) {
			...
			cc.data(circleData).attr('transform', function (d) {
	...
	d3.select('.spark-overlay').on('mousemove', mousemove);
};
...
selectForm = function (ff) {
	...
	// create selection:
	var controlVar = d3.select('#control').append('div').attr('class', 'form-group ccvar');

	// add label:
	var label = d3.select('.ccvar').append('label').html('Variable to display:');
	...
	//add title:
	var formTitle = selection.append('p').attr('class', 'form-title').html('Variable to display');
};
...
function changeVar() {
	...
	if (yy == newY) {
		...
		d3.selectAll('.sparkRegion').classed('hidden', false);
	} else {
		d3.selectAll('.sparkRegion').classed('hidden', true);
	}
...
addSlider = function () {
	...
	var seqVar = chart.seqVar[0],
		...
		range = d3.extent(chart.timeData, function (d) {
			return d[seqVar];
		}),
		int = chart.int[0];

	// input a slider:
	var sliderDiv = d3.select('#control').append('div').attr('class', 'slider-div');

	var sliderTitle = sliderDiv.append('label').html('By ' + seqVar + ':');

	var sliderVal = sliderDiv.append('div').attr('class', 'slider-val').html(current);
...
addZoom = function (chart) {
	...
	d3.select(bg).attr('class', 'background').call(zoom);
};
