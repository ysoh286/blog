...
d3.select(panel).selectAll('polygon').attr('class', 'histBar');
...
d3.selectAll('.histBar')
	...
	.on('click', function (d, i) {
		...
		d3.select('.brush-info').classed('hidden', true);
...
var tt = d3.select('#control').append('p').attr('class', 'brush-info');

//BRUSH:
var brush = d3.brushX().on('start', brushstart).on('brush', brushmove).on('end', brushend);

var pp = panel.parentNode.parentNode;
d3.select(pp).insert('g', 'g:first-child').attr('class', 'brush').call(brush);

// make handles invisible:
d3.selectAll('.handle').style('opacity', 0);
d3.selectAll('.overlay').style('stroke', 'none');
...
function brushmove() {
	...
	//summation of array:
	var sum = groupN.reduce(function (a, b) {
		return a + b;
	}, 0);
	...
	d3.select('.brush-info')
		...
		.html(
			'Interval Range: <span>' +
				intRange[0] +
				' - ' +
				intRange[intRange.length - 1] +
				'</span> <br> Frequency: <span>' +
				sum +
				',' +
				nProp +
				'</span> <br> No. of intervals: <span>' +
				intervalNo +
				'</span>'
		);
}
...
//Reset Button:
reset = function () {
	d3.select(panel).selectAll('.histBar').attr('class', 'histBar');
