...
two way bar plots.
 Code is split in 3 sections: table properties (from line 25),
                              bar and label properties (from line 216),
...
for (i = 1; i <= cellNo; i++) {
	if (i % ncol == 0) {
		...
		// for two way bar plots - different table
		td[i - 1].setAttribute(
			'id',
			'td' + ((i - (i % ncol)) / ncol + 1 + (nrow - 1) * ((i % ncol) - 2))
		);
	} else {
		if (i < ncol - 1) {
		...
		for (j = 1; j <= cellNo / ncol; j++) {
			...
			for (i = 1; i <= cellNo - 2 * (nrow - 1); i++) {
				...
				if (td.innerHTML.indexOf('.') >= 0 && td.innerHTML.indexOf('%') == -1) {
					...
					if (
						tr.contains(document.getElementById('tc' + j)) &&
						tr.contains(document.getElementById('td' + i))
					) {
...
if (rect.x.baseVal.value < 48) {
	//there's some redundant rectangle that appears on two-way bar plots.
	...
	for (j = 1; j < count; j++) {
		...
		if (prop[0].Var1 != undefined) {
			...
			if (i == j) {
				...
				for (k = 0; k <= 1; k++) {
					//highlights both counts and percentage at the same time
